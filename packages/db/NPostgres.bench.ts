...
Deno.bench('NPostgres.event', async (b) => {
...
Deno.bench('NPostgres.event with many tags', async (b) => {
  const secretKey = generateSecretKey();
...
Deno.bench('NPostgres.query by kind', async () => {
  await db.query([{ kinds: [1], limit: 20 }]);
});

Deno.bench('NPostgres.query by multiple kinds', async () => {
...
Deno.bench('NPostgres.query by multiple tags', async () => {
  await db.query([{
    '#p': ['be49045474d8234adbd38dff67bbb9ae2a6d0696bf809e44e9cd12aac0ea6318'],
...
Deno.bench('NPostgres.query many events by tag', async () => {
  await db.query([{
...
Deno.bench('NPostgres.query by kind and multiple pubkeys', async () => {
  await db.query([{
    kinds: [3],
    authors: [
      '235f0103f48a7c04524d0ab40de8d8549c5563545b9ab21da2949c013c48bffd',
