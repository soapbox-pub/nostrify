...
test('MockRelay', async () => {
  const relay = new MockRelay();

  deepStrictEqual(await relay.count([{ ids: [event1.id] }]), { count: 0, approximate: false });

  await relay.event(event1);

  deepStrictEqual(await relay.count([{ ids: [event1.id] }]), { count: 1, approximate: false });
