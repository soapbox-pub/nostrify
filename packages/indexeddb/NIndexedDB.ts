...
/** The events object store name. */
const EVENTS_STORE = 'events';

/** The schema version installed by {@link NIndexedDB}. */
const DB_VERSION = 1;
...
/**
 * A general-purpose `NStore` backed by IndexedDB, suitable for use as a
 * persistent client-side cache in the browser.
 *
 * Construction is synchronous — pass the name of the IndexedDB database to use
 * and the store opens it (installing its schema) in the background. Every
 * method awaits the connection before touching IndexedDB, so calls made before
 * the open settles simply wait for it:
 *
 * ```ts
 ...
 * ```
 *
 * When IndexedDB is unavailable (e.g. iOS Lockdown Mode, some private-browsing
 * contexts) the store degrades to a no-op: `event()` does nothing and
 * `query()` returns `[]`.
 */
export class NIndexedDB implements NStore {
  ...
  /**
   * A promise resolving to the database connection. Construction kicks off the
   * open and stores the still-pending promise here; every method awaits it via
   * {@link getDB} before touching IndexedDB, so the store can be constructed
   * synchronously and used immediately. Resolves to `null` if IndexedDB is
   * unavailable or the open fails, in which case every method silently
   * degrades.
   */
  private readonly db: Promise<IDBPDatabase<EventsDB> | null>;

  /**
   * Create a store backed by the IndexedDB database named `name`, opening it
   * (and installing the schema) in the background.
   *
   * This is synchronous: no `await` is required to construct the store. The
   * connection opens concurrently and every method waits for it before use, so
   * a call made before the open settles just waits for it to finish.
   */
  constructor(name: string, opts: NIndexedDBOpts = {}) {
    this.db = NIndexedDB.openDatabase(name);
    this.indexTags = opts.indexTags ?? NIndexedDB.indexTags;
  }

  /**
   * Open the events database, installing the object store and indexes the
   * store relies on. Resolves to `null` when IndexedDB is unavailable (or the
   * open fails) so callers can short-circuit to a no-op.
   */
  private static async openDatabase(name: string): Promise<IDBPDatabase<EventsDB> | null> {
    if (typeof indexedDB === 'undefined') return null;
    try {
      return await openDB<EventsDB>(name, DB_VERSION, {
        upgrade(db) {
   ...
   */
  private getDB(): Promise<IDBPDatabase<EventsDB> | null> {
    return this.db;
  }
  ...
  /**
   * Close the underlying database connection, releasing its handle. Useful in
   * tests and short-lived processes so an open connection doesn't keep the
   * runtime alive. Construct a new store to use the database again.
   */
...
// ============================================================================
// ParsedFilter — a parsed, normalized representation of a single Nostr filter,
...
// (see the planner above). The matching semantics are identical to NIP-01:
