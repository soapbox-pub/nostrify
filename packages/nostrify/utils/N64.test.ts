import { test } from "node:test";
import { deepStrictEqual } from "node:assert";

import event0 from "../../../fixtures/event-0.json" with { type: "json" };

import { N64 } from "./N64.ts";

await test("N64", () => {
  const base64 = N64.encodeEvent(event0);
  const event = N64.decodeEvent(base64);

  deepStrictEqual(event, event0);
});

await test("N64.encodeEvent", () => {
  const base64 = N64.encodeEvent(event0);
  deepStrictEqual(
    base64,
    "",
  );
});

await test("N64.encodeEventUrl", () => {
  const base64url = N64.encodeEventUrl(event0);

  // Base64url alphabet only, no padding.
  deepStrictEqual(/^[A-Za-z0-9_-]+$/.test(base64url), true);

  // Round-trips back to the original event.
  deepStrictEqual(N64.decodeEvent(base64url), event0);
});

await test("N64.decodeEvent", () => {
  const event = N64.decodeEvent(
    "",
  );
  deepStrictEqual(event, event0);
});
