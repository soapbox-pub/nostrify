...
> {
  ...
  return {
    ...
    [Symbol.asyncDispose]: async () => {
      await withoutDebug(() =>
        kysely.schema.dropTable("nostr_events").ifExists().cascade().execute()
      );
...
await test("NPostgres.migrate", { skip: !databaseUrl }, async () => {
  await using _db = await createDB();
});

await test("NPostgres.migrate twice", { skip: !databaseUrl }, async () => {
...
await test("NPostgres.count", { skip: !databaseUrl }, async () => {
  await using db = await createDB();
  const { store } = db;
...
await test("NPostgres.query", { skip: !databaseUrl }, async () => {
...
await test(
  "NPostgres.query with multiple tags",
  { skip: !databaseUrl },
  async () => {
    await using db = await createDB();
    const { store } = db;
...
await test("NPostgres.query with multiple tags and time window", {
  skip: !databaseUrl,
}, async () => {
  await using db = await createDB();
  const { store } = db;
...
await test("NPostgres.query with multiple tags and time window and limit", {
  skip: !databaseUrl,
}, async () => {
  await using db = await createDB();
  const { store } = db;
...
await test("NPostgres.query with search", { skip: !databaseUrl }, async () => {
  await using db = await createDB();
  const { store } = db;
...
await test("NPostgres.remove", { skip: !databaseUrl }, async () => {
  await using db = await createDB();
  const { store } = db;
...
await test("NPostgres.event with timeout", { skip: !databaseUrl }, async () => {
  await using db = await createDB();
  const { store } = db;
...
await test("NPostgres.event with the same event multiple times", {
  skip: !databaseUrl,
}, async () => {
  await using db = await createDB();
  const { store } = db;
...
await test("NPostgres.event with parameterized replaceable event", {
  skip: !databaseUrl,
}, async () => {
  await using db = await createDB();
  const { store } = db;
...
await test("NPostgres.event with a parameterized-replaceable deleted event", {
  skip: !databaseUrl,
}, async () => {
  await using db = await createDB();
  const { store } = db;
...
await test("NPostgres.event does not delete another user's event", {
  skip: !databaseUrl,
}, async () => {
  await using db = await createDB();
  const { store } = db;
...
await test("NPostgres.transaction", { skip: !databaseUrl }, async () => {
  await using db = await createDB();
  const { store } = db;
...
await test("NPostgres timeout", { skip: !databaseUrl }, async (t) => {
  await using db = await createDB();
...
await test("NPostgres.req streams events", { skip: !databaseUrl }, async () => {
  await using db = await createDB();
  const { store } = db;
...
await test("NPostgres.req with timeout", { skip: !databaseUrl }, async () => {
  await using db = await createDB();
  const { store } = db;
...
await test("NPostgres.shouldOrder", () => {
...
await test("NPostgres search extensions", { skip: !databaseUrl }, async () => {
  await using db = await createDB({
...
await test("NPostgres search extensions with multiple keys and values", {
  skip: !databaseUrl,
}, async () => {
  await using db = await createDB({
    indexExtensions(event) {
      ...
      if (imeta.length) {
        ...
        if (
          imeta.every((tags) =>
            tags.some(([name, value]) =>
              name === "m" && value.startsWith("video/")
            )
          )
        ) {
          ext.video = "true";
...
await test("NPostgres search extensions with multiple negative tokens", {
  skip: !databaseUrl,
}, async () => {
  await using db = await createDB({
    indexExtensions(event) {
