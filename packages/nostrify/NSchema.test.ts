...
test('n.bech32', () => {
  ok(n.bech32('npub').safeParse('npub108pv4cg5ag52nq082kd5leu9ffrn2gdg6g4xdwatn73y36uzplmq9uyev6').success);
  ok(
...
test('n.metadata', () => {
  ...
  deepStrictEqual(n.metadata().parse({ about: 'I am a developer.' }).about, 'I am a developer.');
  deepStrictEqual(n.metadata().parse({ picture: 'https://nostrify.dev/1.png' }).picture, 'https://nostrify.dev/1.png');
  deepStrictEqual(n.metadata().parse({ banner: 'https://nostrify.dev/2.png' }).banner, 'https://nostrify.dev/2.png');
  deepStrictEqual(n.metadata().parse({ nip05: 'alex@gleasonator.dev' }).nip05, 'alex@gleasonator.dev');
  deepStrictEqual(n.metadata().parse({ lud06: 'lnurl1acdacd' }).lud06, 'lnurl1acdacd');
  deepStrictEqual(n.metadata().parse({ lud16: 'alex@alexgleason.me' }).lud16, 'alex@alexgleason.me');
  deepStrictEqual(n.metadata().parse({ website: 'https://nostrify.dev' }).website, 'https://nostrify.dev');
  ...
  deepStrictEqual(n.metadata().parse({ nip05: 'nostrify.dev' }).nip05, undefined);
  deepStrictEqual(n.metadata().parse({ lud06: 'npub1abc' }).lud06, undefined);
  deepStrictEqual(n.metadata().parse({ lud16: 'nostrify.dev' }).lud16, undefined);
  deepStrictEqual(n.metadata().parse({ website: 'nostrify.dev' }).website, undefined);
});
